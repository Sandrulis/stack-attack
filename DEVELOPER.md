...
Tables `player_profiles` and `player_play_days` use RLS deny; the client goes through `ensure_player`, `set_player_name`, `start_player_run`, `finish_player_run`, `spend_player_life`, `player_leaderboard`. Hearts (lives) sit on `player_profiles.lives` / `next_life_at`. New players start with 1 heart and refill one every 24 hours.
